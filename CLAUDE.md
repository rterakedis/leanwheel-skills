...
**Model Routing** (not in upstream): Apple/Swift projects only (`swift_project` = `docs/setup/swift/` exists OR an `.xcodeproj`/`.xcworkspace`/`Package.swift` present). When on, Activation announces a per-story model plan and the loop issues **MODEL SWITCH GATE** hard-stops before each phase — Sonnet (medium) for create-story, Opus (medium; high for concurrency/complex state) for dev-story, Sonnet (high) for code-review — waiting for the user to switch model in the UI and type "ready" (the model can't switch itself). Two switches per story; a gate is skipped when the target already matches the last model set, and the user can opt out for the run ("just keep going"). Non-Swift projects keep the gates **off** and run fully automated (Sonnet/Haiku handle Python/web well) — only a one-line note at start. Rationale: switching is pure friction where the cheaper model is already capable; it only pays off on Swift, where Opus's higher per-attempt accuracy minimizes Build & Test Gate iterations.
