...
**When invoked from `/dev-story`:** leave mismatches in `### Design Verification` — the independent reviewer (code-review Pass E) triages them into `patch`, `fix-now`, or `decision-needed` findings like any other. The story does not reach `done` with an unresolved HIGH design finding.
