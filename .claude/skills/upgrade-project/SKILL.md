...
| Status script | `scripts/status.sh` present + executable |
| Status line | `statusLine` key present in `.claude/settings.json`. A `statusLine` that points at something other than `status.sh` is the user's own — report it as **present**, never as missing (DD-69) |
...
5. **Scripts:** if `scripts/commit-push.sh` is missing, copy from
   ...
   and `{skills_path}/scripts/status.sh` if `scripts/status.sh` is missing and `chmod +x`.
   With `status.sh` in place, add the status line to `.claude/settings.json` **only if the
   file has no `statusLine` key at all**:
   `{ "statusLine": { "type": "command", "command": "./scripts/status.sh --line", "padding": 0 } }`.
   An existing `statusLine` is a personal preference — leave it and say so in the report.
