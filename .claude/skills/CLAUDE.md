...
| `story-flywheel` | Epic number from milestone *title*; `gh api --jq` filtering; **Subagent delegation + model/effort routing table — canonical** (all three pinned Sonnet; dev-story gets a `model: opus` override on Swift only — Opus is the flywheel's cost ceiling, a Fable session is never inherited; Haiku docs-sync; effort pinned per runner, never inherited; no mid-session model or effort switching — both bust the prompt cache); **non-return + PID/artifact wait rule — canonical**; Phase 4 checkpoint with VERIFICATION + TESTING PLAN shape; ledger roll-up via `ledger.sh` every story; auto-pilot option; inline-on-session-model fallback. | DD-20 DD-21 DD-62 DD-73 |
