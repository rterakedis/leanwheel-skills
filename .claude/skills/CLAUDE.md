...
| `next` / `status` | Zero-token state detection routed on the readiness / retro stamps; `status` points at `/next`. `status` **runs `scripts/status.sh`** rather than re-deriving counts in-model, and keeps the pre-DD-69 `gh` path only as the fallback for projects without the script. | DD-69 |
...
- **Scripts**: `gh-track.sh` (tracking), `sabotage.sh` (prove a gate can fail), `ledger.sh` (normalized ledger appends + verify-green enforcement), `status.sh` (project status; reads frontmatter, not GitHub — `--line` feeds the status line, `--drift` is the only networked mode), `sim.sh`, `commit-push.sh`. Executed, never read into context. Build/test gates run quiet and `tee` full logs to the self-ignoring `.leanwheel/logs/` (DD-63).
...
- **Status line** (`scripts/status.sh --line`, `statusLine` in project `.claude/settings.json`): the always-on half of `/status`, wired by `setup` Step 4 and `upgrade-project`. Reads story frontmatter only — no `gh`, no network, no cache (measured ~40ms at 300 stories, against Claude Code's 300ms debounce), so it cannot go stale or block a render. Both skills **never overwrite an existing `statusLine`**. DD-69.
