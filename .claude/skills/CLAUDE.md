...
| `setup` | Multi-select Apple platforms + web surface questions; copies Swift stubs (incl. `testability.md`, `simulator.md`), web stubs, `simplicity.md` pointer (never inlined), hooks, `sim.sh`/`gh-track.sh`/`sabotage.sh`, evals/metrics dirs; writes `.leanwheel/manifest.json`. Migrate flow re-runs 3s. | DD-34 DD-40 DD-53 DD-60 |
...
| `dev-story` | Task/AC check-offs during work; guidance routing by topic; Design Contract as source of truth; **Build & Test Gate by running** + evals RUN/BUILD + fail-first (`sabotage.sh`); invariant evidence; design-verify for UI; inline review Passes A–F incl. `fix-now`; **Testing Plan report field (`AUTOMATED:` / `MANUAL:`) — canonical**; reports `INFRA TOUCHED` instead of running docs-sync under a flywheel; ledger line. | DD-10 DD-11 DD-12 DD-31 |
| `code-review` | Clean-review line; epic-context learnings (never stubs a missing cache); Pass E design/dark-pattern + identifier checks; Pass F over-engineering; component inventory; **Verify green** (build+test+evals) after patches; SCORE rubric; DRIFT flag; **`[Fix-Now]` ceiling — canonical**. | DD-11 DD-12 DD-23 DD-56 |
...
| `story-flywheel` | Epic number from milestone *title*; `gh api --jq` filtering; **Subagent delegation + model routing table — canonical** (Sonnet default, Opus only for dev-story on Swift, Haiku docs-sync); **non-return + PID/artifact wait rule — canonical**; Phase 4 checkpoint with VERIFICATION + TESTING PLAN shape; ledger roll-up; auto-pilot option; MODEL SWITCH GATE fallback. | DD-20 DD-21 |
...
- **Scripts**: `gh-track.sh` (tracking), `sabotage.sh` (prove a gate can fail), `sim.sh`, `commit-push.sh`. Executed, never read into context.
- **Agents** (`agents/`): `lw-story-creator` (Sonnet), `lw-story-developer` (Sonnet; Opus on Swift by override), `lw-story-reviewer` (Sonnet), `lw-docs-sync` (Haiku, `effort: low`). Each carries a one-line non-return reinforcement beside its report contract. **New agent or skill → re-run the symlink sync** (root CLAUDE.md).
- **Ledger** (`stubs/metrics/`): `docs/metrics/flywheel-ledger.jsonl`, one line per phase per story, single-redirect append, rotated by CUT-RELEASE.
