...
**Effort routing (second axis, static):** effort is the 5-series cost control — it governs *all* output tokens, thinking and tool calls alike — and it is set only via `effort:` frontmatter in the agent defs (the Agent tool has no per-spawn override, so the dynamic-Swift exception above can't be replicated for effort). Every phase-runner pins it explicitly, because a subagent spawn is its own conversation: pinning costs the orchestrator no cache, while *inheriting* would let a `max`-effort session leak past the model ceiling into every phase. Pins: creator `medium` (authoring from a cache is synthesis, not deep reasoning), developer and reviewer `high` (implementation and adversarial reading are exactly what `high` is for — and stepping either down would also make the model take *fewer tool calls*, which is wrong for phases that must run gates), `lw-docs-sync` `low`. Do not step the developer down to buy budget — use the model axis, which is what it's for. Anthropic's guidance is to sweep effort against your own evals rather than trusting inherited levels; `evals/` is where that sweep belongs.
...
## Fallback: subagents unavailable

Run every phase inline on the **current session model**. Do not issue model-switch prompts: changing `/model` (or `/effort`) mid-conversation busts the prompt cache, so a per-phase Sonnet→Opus→Sonnet dance costs more than it saves. If the user wants the Swift dev phase on a stronger model than the session is running, tell them once at activation — "start the flywheel in a fresh session on {model} for this epic" — and proceed on the current model if they decline. Inline phases execute `skills/create-story/SKILL.md`, `skills/dev-story/SKILL.md`, and `skills/code-review/SKILL.md` directly; the checkpoint, gates, and report fields are unchanged.
...
**Fallback mode:** execute `skills/create-story/SKILL.md` inline on the session model.
...
MANUAL: {only what no test can exercise — one line per item, `{step} → {expected} [visual-judgment | device-only | sandbox-only | setup-unreachable]`, with the exact setup command (all flags) when one is needed. "none — fully automated" or "none — no user-visible surface changed." Both sub-fields are mandatory — see dev-story → Testing Plan (required report field).}
