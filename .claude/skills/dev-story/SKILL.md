...
3. **Run the cumulative eval set.** If `docs/evals/` exists, run `bash scripts/evals.sh --epic {n} --quiet` (zero-token — the script collects, batches and scores the accumulated `type: command` cases; its last line is the RUN report, per evals → RUN). Do not read `docs/evals/*.md` into context to do this. A failing case is a **regression** of an earlier story — treat it exactly like a red build: fix and re-run, or HALT. This is what makes the regression net *cumulative* across stories, not just per-story.
