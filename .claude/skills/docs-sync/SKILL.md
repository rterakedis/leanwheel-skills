...
Derive the changed-file list with `git diff --name-only` against the last commit (or the merge-base of the current branch), then run **OPERATIONAL** over it and report what changed. Use this to catch the human guides up after manual work done outside the flywheel. To keep it off the session model, call the Agent tool with `subagent_type: "lw-docs-sync"` (Haiku) and a prompt naming op `OPERATIONAL` plus the derived file list, rather than running inline (fallback: inline).
