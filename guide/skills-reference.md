...
| Invocation | What it does |
|------------|-------------|
| `/story-flywheel` | Fully automated create → dev → review loop. Spawns isolated subagents (`lw-story-creator` → `lw-story-developer` → `lw-story-reviewer`) so each phase's heavy reads stay in a throwaway context, keeping the main thread lean; model routing is automatic (Sonnet for create/review, Opus for Swift dev passes). A subagent report missing its required fields is treated as a non-return — the orchestrator resumes the agent rather than advancing on a narration-only message. Pauses for human decisions surfaced by the Clarification Gate in Phase 1. Fallback when subagents are unavailable: **MODEL SWITCH GATE** hard-stops (Swift only) with a per-story model plan; user types "ready" after switching in the UI. |
